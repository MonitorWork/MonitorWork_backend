...
app.post("/professores", async (req, res) => {
  try {
    const { nome, age, telefone, universityId, disciplina, email, password } =
      req.body;
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Nome, email, senha, ID da universidade, disciplina e telefone são obrigatórios",
      });
    }
    ...
    if (professorExistente) {
      return res
        .status(400)
        .json({ error: "Já existe um professor com esse e-mail ou telefone" });
    }
...
app.put("/professores/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao atualizar professor" });
  }
});
